// src/modules/admin/AdminEditReportPage.tsx
...
export const AdminEditReportPage: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        // ---------- 1) Load student list for selector ----------
        ...
        // ---------- 3) Resolve existing attachments ----------
        ...
        console.error("Unexpected error loading admin report", err);
        setError("Could not load report.");
      } finally {
        setLoading(false);
      }
  ...
  const handleToggleExistingPublic = (idx: number, value: boolean) => {
    setExistingAttachments((prev) =>
      prev.map((a, i) => (i === idx ? { ...a, isPublic: value } : a))
    );
  };
  ...
  const navigateBackToStudent = () => {
    ...
    const targetStudentId = studentFromQuery || studentId || report.student_id;
    ...
    <Stack>
      <Group justify="space-between">
        <div>
          ...
          <Text size="sm" c="dimmed">
            Adjust all report details, including grades, comments, and photos.
            Changes will be reflected in teacher dashboards and overdue
            calculations.
          </Text>
        </div>
        <Button
      ...
      <Card withBorder>
        <form onSubmit={handleSubmit}>
          <Stack gap="md">
            {/* Student selector (admin can reassign) */}
            <Select
              label="Student"
            ...
            <Group grow>
              <TextInput
                label="Grade (short)"
                placeholder="Overall grade / GPA (e.g., A / B / Pass / Good / 3.5)"
                value={grade}
            ...
            <Textarea
              ...
              description="Visible to donors; used in donor reporting."
