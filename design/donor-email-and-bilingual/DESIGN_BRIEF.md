...
- **Per-recipient delivery** (added from the donor-records grill, 2026-09-11). `sent_to` is a snapshot of who received the report; it can't express "sent to two donors, one still waiting". A `report_deliveries` table holds one row per report and donor: `report_id`, `donor_id`, `email` (the address actually used), `language`, `status` (`waiting` · `sent` · `failed`), `sent_at`, `error`, `saved_to_donor`, `created_by`, plus the standard timestamps and `deleted_at` (TEXT, `uuid_generate_v4()`, no FKs, per `SCHEMA.md`). `sent_at` and `sent_to` on the report stay, derived from the delivery rows, so every existing reader keeps working.
- **Added recipients** (decided 2026-09-11) are recorded like donors, in `sent_to` and `report_deliveries`, with `donor_id` null and the address and language used, so a report's history shows everyone it reached.
...
| `EmailPreviewDialog` | Modify | The exact email, rendered in an iframe, in the donor's language, with the recipients and reply-to named. Holds the real send button. **No longer all-or-nothing (done 2026-09-11):** donors with an address are sent to, and donors without one are named in a warning before sending and in a notice after. **Still to build (decided 2026-09-11):** an **Add recipients** field at the top for extra one-off addresses, each with an EN · ไทย switch defaulting to English; an inline **Add their address** box with **Save to donor** ticked for each sponsor with no email; Send disabled until at least one address is set; and the *Waiting* state for anyone still skipped. |
| `report-email` edge function | Modify | Renders and sends via Resend, following `send-donor-card`, and degrades with a legible message when the key is unset. **Done 2026-09-11:** it sends to every donor who has an address, skips and names those without one (in the response and in `send_error`), doesn't re-email donors already in `sent_to`, and no longer erases `sent_at` / `sent_to` on a failed attempt. **Still to build:** accepting `extraRecipients: [{ email, language }]` and `donorAddresses: [{ donorId, email, saveToDonor }]` alongside `reportId`; sending one waiting delivery; and a `report_deliveries` row per recipient. Every address is validated server-side as well as in the form. |
| `AdminReportsBetaPage` (Reports beta) | Modify | The real reports list; "Field reports" at `/admin/reports` is a query builder. It shows no send state today, so the *Approved · not sent* state designed below was never built there. It gains a **Waiting to send** filter (approved reports with any `waiting` or `failed` delivery), a *Waiting for* column naming the donors, and a count on its sidebar item. |
...
**When a donor has no email (drafts).** Approving a report sends it straight away to every funding donor who has an address and **Send report emails** on. A donor with no address no longer blocks the others; this was fixed ahead of the feature on 2026-09-11, and `report-email` now skips and names them. With drafts built, that donor also gets a delivery row with status `waiting`, and the approval confirmation names them: *"Sent to Acme Foundation and Somchai P. Waiting: Nong B. family (no email)."*

**Sending a waiting report.** **Reports beta → Waiting to send** lists every approved report with a waiting or failed delivery, oldest first, naming who it's waiting for. Opening one shows the draft: the rendered email in that donor's language, a **Send to** address field, and **Also save this address to {donor}**, ticked by default (decided). The address is validated like the donor form's email field. **Send** emails that one donor only; donors already sent to aren't emailed again. With the box ticked, the address is written to the donor, so their next report goes out automatically. Unticked, it's a one-off, and the next report waits again. A report leaves the list once nothing on it is waiting.

**Adding recipients at send** (decided 2026-09-11). The send dialog opens with an **Add recipients** field at the top. Any address can be added for this send only, such as a relative or a teacher; it's validated like the donor form's email field and isn't saved anywhere. Each added address has an **EN · ไทย** switch, starting on English, and Send is blocked if that language's version is empty, the same rule donors follow. A sponsor with no email shows an inline **Add their address** box with **Save to donor** ticked, so they're covered in the same step. **Send** stays disabled until at least one address is set. A no-email sponsor whose box is left empty is still sent nothing, and their report waits in **Waiting to send** (decided: keep both).
